...
class Start:
    ...
    def __init__(self, master, start_callback=None, leaderboard=None):
        """Initializes the start screen with a welcome label, difficulty selection, and a start button. (Leaderboard hidden)"""
        ...
        # Leaderboard is now hidden on the start screen
        ...
        #self.difficulty_segmented.grid(row=0, column=1, padx=10, pady=5, columnspan=2)
...
class GUI:
    ...
    def __init__(self):
        ...
        try:
            ...
            print(f"Warning: Could not set main window icon (flag.ico): {e}")
    ...
    def show_question(self, correct_last=None):
        ...
        self.game_screen = GameGUI(
            ...
            mode=mode, answer_checker=answer_checker, correct_text=correct_text, hearts_imgs=heart_pil_imgs,
    ...
    def show_final_score(self):
        ...
        label.pack(pady=20)
        # Leaderboard is now hidden on the end screen
        leaderboard = self.highscore_manager.get_leaderboard()
        min_score_to_qualify = -1
        if leaderboard:
             min_score_to_qualify = leaderboard[-1][0] if len(leaderboard) >= 10 else -1
        is_new_high = self.score_manager.get_score() > min_score_to_qualify or (len(leaderboard) < 10 and self.score_manager.get_score() >=0)
        name_widgets = []
        ...
        view_btn = ctk.CTkButton(frame, text="View Results", font=("Arial", 16), command=self.view_results)
    ...
    def view_results(self):
        ...
        try:
            ...
            print(f"Warning: Could not set icon for results window (flag.ico): {e}")
            pass
        results_window.geometry("700x600")
        title_label = ctk.CTkLabel(results_window, text="Maori Quiz Results", font=("Arial", 24, "bold"))
        title_label.pack(pady=(15, 5))
        text_widget = ctk.CTkTextbox(results_window, font=("Consolas", 14), wrap="word")
    ...
    def _get_results_text(self):
        ...
        for idx, result in enumerate(self.answered_questions, 1):
            ...
            if self.difficulty == "hard":
                user_ans_display = str(user_ans_val).strip() if user_ans_val is not None else ""
                ...
                for i, choice_text in enumerate(choices):
                    ...
                    if i == correct_idx:
                        marker += ' (Correct)'
                    lines.append(f"   {i+1}. {choice_text} {marker.strip()}")
            lines.append("")
        return "\n".join(lines)
    ...
    def export_results(self):
        ...
        try:
            ...
            for widget in self.root.winfo_children():
                if isinstance(widget, ctk.CTkFrame) and widget.winfo_ismapped():
                    is_final_score_frame = False
                    for child in widget.winfo_children():
                        if isinstance(child, ctk.CTkLabel) and "Game Over!" in child.cget("text"):
                    ...
                    if is_final_score_frame:
                        ...
                        break
            if current_frame:
                export_label = ctk.CTkLabel(current_frame, text=f"Results exported to {filename}", font=("Arial", 16), text_color="green")
                export_label.pack(pady=10)
                current_frame.after(3000, export_label.destroy)
            else:
                print(f"Results exported to {filename}. (UI feedback failed to display)")
        except Exception as e:
