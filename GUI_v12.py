...
class GUI:
    ...
    def view_results(self):
        results = self._get_results_text()
        results_window = ctk.CTkToplevel(self.root)
        results_window.title("Maori Quiz Results")
        results_window.geometry("700x600")
        try:
            # Use iconbitmap only if running on Windows and file exists
            import os
            if os.name == 'nt' and os.path.exists("flag.ico"):
                results_window.iconbitmap("flag.ico")
        except Exception:
            pass
        # Make sure the window stays on top and grabs focus
        ...
        # Remove topmost after focus to allow normal stacking
        results_window.after(500, lambda: results_window.attributes('-topmost', False))
    ...
    def _get_results_text(self):
        ...
        for idx, result in enumerate(self.answered_questions, 1):
            ...
            if self.difficulty == "hard":
                ...
                lines.append(f"  Correct answer: {choices[correct]}")
                lines.append("")
            else:
                for i, choice in enumerate(choices):
                    ...
                    lines.append(f"   {i+1}. {choice} {marker}")
                lines.append("")
        return "\n".join(lines)
